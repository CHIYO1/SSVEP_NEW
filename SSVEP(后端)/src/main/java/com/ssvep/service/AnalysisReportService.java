...
public class AnalysisReportService {
    ...
    public List<AnalysisReportDto> getReportByTestRecord(Long recordId) {
        List<AnalysisReports> report = reportsDao.getReportByTestRecord(recordId);
        if(report == null||report.isEmpty()){
            return Collections.emptyList();
        }
        List<AnalysisReportDto> result = report.stream().map(this::convertToDto).collect(Collectors.toList());
        return result;
    }
